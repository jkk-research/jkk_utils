...
def generate_launch_description():
    ...
    """
    lexus_ros_pkg_dir = get_package_share_directory('drone_bringup')
    default_params_file = \
        Path(lexus_ros_pkg_dir) / 'launch' / 'drivers' / 'ouster_config.yaml'
    params_file = LaunchConfiguration('params_file')
    params_file_arg = DeclareLaunchArgument('params_file', default_value=str(default_params_file),description='name or path to the parameters file to use.')

    ouster_ns = LaunchConfiguration('ouster_ns')
    ouster_ns_arg = DeclareLaunchArgument('ouster_ns', default_value='drone1/os')
    ...
    sensor_activate_event = RegisterEventHandler(
        OnStateTransition(
            ...
            entities=[
                LogInfo(msg="os_driver activating... drone1/os"),
